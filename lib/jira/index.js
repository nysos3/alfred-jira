...
Jira = {
  ...
  'listAll': function(bookmarkConfig) {
    let self = this;
    bookmarkConfig = bookmarkConfig.trim()
    bookmarkConfig = bookmarkConfig.replace(/^[\s+]?★[\s+]?/, '')
    config.bookmarks.forEach(function(i, v) {
      if (i.name == bookmarkConfig) {
        bookmarkConfig = `bookmark-${v}`;
      }
    })
    return new Promise((resolve, reject) => {
      self.auth()
