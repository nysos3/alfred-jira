...
function validateRepo() {
  ...
  if (exists) {
    ...
    if (!/nysos3/.test(pushUrl)) {
      sh.execSync("git remote set-url origin git@github.com:nysos3/alfred-jira.git");
    ...
    return new Promise((resolve, reject) => {
      ...
      if (cache.get('update', cacheLimit) === undefined && !updates) {
        try {
          ...
          if (/Permission denied \(publickey\)/.test(e)) {
            // Use https instead.
            sh.execSync('git remote set-url origin https://github.com/nysos3/alfred-jira.git');
          }
        }
      }
